...
const CONF_LABEL = { insufficient: "No evidence yet", low: "Low confidence", moderate: "Moderate confidence", high: "High confidence" } as const;

export function CapabilityGrid({ progress, compact = false }: { progress: AreaProgress[]; compact?: boolean }) {
  return (
...
export function AreaCard({ p }: { p: AreaProgress }) {
  ...
  const Dir = p.recent_direction === "improving" ? ArrowUpRight : p.recent_direction === "declining" ? ArrowDownRight : ArrowRight;
  return (
    <div className="rounded-xl border border-border bg-surface p-4">
      <p className="text-sm font-medium">{AREA_LABELS[p.capability_area]}</p>
      <p className={`mt-2 font-display text-xl ${building ? "text-muted-foreground" : "text-foreground"}`}>
        {STAGE_LABELS[p.mastery_stage]}
      </p>
      {building ? (
        <p className="mt-1 text-xs text-muted-foreground">{p.total_questions} of 5 answers needed</p>
      ) : (
        <>
          <Progress value={p.weighted_score} className="mt-3 h-1.5" />
          <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
            <span>{p.weighted_score}% · {CONF_LABEL[p.evidence_confidence]}</span>
...
export function PrepStatusBadge({ status }: { status: "not_started" | "in_progress" | "completed" }) {
  const map = {
