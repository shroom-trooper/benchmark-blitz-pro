...
import { ArrowRight, CheckCircle2, Flame, Lightbulb, Sparkles, XCircle } from "lucide-react";
import { toast } from "sonner";
import { saveErrorMessage } from "@/lib/errors";
import { AppShell } from "@/components/AppShell";
import { getRecruiterWeek, submitRecruiterWeek } from "@/lib/benchmark.functions";
import { track } from "@/lib/analytics";
import { quarterForWeek, quarterThemesForTrack } from "@/lib/gamification";
...
export const Route = createFileRoute("/_authenticated/recruiter/session/$week")({
  head: () => ({
    meta: [
      { title: "Recruiter simulation · Benchmark" },
      {
        name: "description",
        content:
          "Work through three realistic recruiting scenarios and get instant, evidence-based feedback.",
      },
      { property: "og:title", content: "Recruiter simulation · Benchmark" },
      {
        property: "og:description",
        content: "Three recruiting scenarios, instant feedback, XP and streak rewards.",
      },
...
type SubmitResult = Awaited<ReturnType<typeof submitRecruiterWeek>>;
type WeekData = Awaited<ReturnType<typeof getRecruiterWeek>>;

function RecruiterSession() {
  const { week } = useParams({ from: "/_authenticated/recruiter/session/$week" });
  const weekNumber = Number(week);
  const getWeekFn = useServerFn(getRecruiterWeek);
  const submitFn = useServerFn(submitRecruiterWeek);
  ...
  const query = useQuery({
    queryKey: ["recruiter-week", weekNumber],
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["recruiter-me"] });
      queryClient.invalidateQueries({ queryKey: ["recruiter-week", weekNumber] });
      ...
      <AppShell>
        <div className="rounded-xl border border-border bg-surface p-8 text-center">
          ...
          <Button asChild className="mt-6">
            <Link to="/recruiter">Back to hub</Link>
          </Button>
        </div>
      </AppShell>
    );
  ...
  const theme = quarterThemesForTrack("recruiter")[quarterForWeek(weekNumber)]!;
...
function Header({ week, topic, theme }: { week: number; topic: string; theme: string }) {
  return (
    <div>
      <Badge className="bg-primary/15 text-primary hover:bg-primary/15">
        Recruiter · Week {week} · {theme}
      </Badge>
    ...
    <AppShell>
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="animate-pop rounded-2xl border border-primary/40 bg-gradient-to-br from-primary/15 to-surface p-8 text-center">
          ...
          <h1 className="mt-3 text-3xl">{result.correctCount}/3 correct</h1>
          <p className="mt-1 text-body">Recruiter week {weekNumber} complete</p>
