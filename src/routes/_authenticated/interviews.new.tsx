...
function NewInterview() {
  ...
  const toggle = (c: string) =>
    setComps((xs) => (xs.includes(c) ? xs.filter((x) => x !== c) : [...xs, c]));
  const valid =
    role.trim().length >= 2 &&
    candidate.trim() &&
    when &&
    (stage !== "__custom" || customStage.trim().length >= 2);
      ...
      >
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            The more context you add, the more relevant your preparation.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-surface p-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {[
              ...COMMON_COMPETENCIES,
              ...comps.filter((c) => !(COMMON_COMPETENCIES as readonly string[]).includes(c)),
            ].map((c) => (
              <button
                type="button"
                key={c}
                onClick={() => toggle(c)}
                className={`rounded-full border px-3 py-1 text-sm ${comps.includes(c) ? "border-primary bg-primary/15 text-primary" : "border-border text-muted-foreground hover:text-foreground"}`}
              >
                {c}
              </button>
            ))}
          ...
          <div className="mt-3 flex gap-2">
            ...
            <Button
              ...
              variant="outline"
              onClick={() => {
                const c = customComp.trim();
                if (c && !comps.includes(c)) setComps([...comps, c]);
                setCustomComp("");
              }}
            >
              Add
        ...
        <div className="space-y-4 rounded-xl border border-border bg-surface p-5">
          ...
          <Field label="Your responsibility in this interview">
            <Textarea
              value={resp}
              onChange={(e) => setResp(e.target.value)}
              rows={2}
              placeholder="e.g. Assess collaboration and stakeholder management"
            />
          </Field>
          ...
          <Field label="Candidate CV notes">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Used only to create fair, job-relevant practice scenarios. Only you can see it — group
              leads cannot.
            </p>
          </Field>
          <Field label="Company interviewing principles (one per line)">
            <Textarea value={principles} onChange={(e) => setPrinciples(e.target.value)} rows={3} />
          </Field>
        </div>
